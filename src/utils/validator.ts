...
export const isId = (value: string, {min = 4, max = 19} = {}) => {
...
export const isPhoneNumber = (value: string, withoutHyphen = false) => {
  if (withoutHyphen && value.length < 12) {
    const splitedArr = value.split('');
    const firstNumber = splitedArr.splice(0, 3).join('');
    const lastNumber = splitedArr.splice(value.length - 4).join('');
    const middleNumber = splitedArr.join('');
...
export const isCheck = (value: string, checkType = types[0]) => {
  let regExp = null;
